...
const ReclamationOffcanvas = ({ location }: ReclamationOffCanvasProps) => {
  ...
  const fazerReclamacaoButtonRef = useRef<HTMLButtonElement>(null);
  const listarReclamacoesButtonRef = useRef<HTMLButtonElement>(null);
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      if (event.key === 'Tab') {
        ...
        if (fazerReclamacaoButtonRef.current === document.activeElement) {
          listarReclamacoesButtonRef.current?.focus();
        } else {
          fazerReclamacaoButtonRef.current?.focus();
        }
      }
    };
  ...
  return (
    <div style={{ display: 'flex', gap: '10px' }}>
      {!isCompany &&
        <button
          ref={fazerReclamacaoButtonRef}
          className="btn btn-primary mb-3"
          type="button"
          data-bs-toggle="offcanvas"
          data-bs-target="#offcanvasReclamation"
          aria-controls="offcanvasReclamation"
        >
          Fazer reclamação
        </button>
      }
